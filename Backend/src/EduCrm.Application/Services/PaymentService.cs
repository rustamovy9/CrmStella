    ...
    ILogger<PaymentService> logger) : IPaymentService
{
    ...
    public async Task<Result<StudentBalanceResponse>> GetStudentBalanceAsync(int studentId)
    {
        ...
        if (student is null)
        {
            ...
            return Result<StudentBalanceResponse>.Fail("Student not found", ErrorType.NotFound);
        }
        ...
        int createdByUserId)
    {
        ...
        if (student is null)
        {
            ...
            return Result<PaymentResponse>.Fail("Student not found", ErrorType.NotFound);
        }
        ...
        if (group is null)
        {
            ...
            return Result<PaymentResponse>.Fail("Group not found", ErrorType.NotFound);
        }
        ...
        if (request.Receipt is not null && request.Receipt.Length > 0)
        {
            ...
            catch (ArgumentException ex)
            {
                ...
                // Not critical — payment is saved, receipt skipped
            }
        }
    ...
    private static PaymentListItemResponse MapToListItem(Domain.Entities.Payment p) => new()
    {
        Id              = p.Id,
        StudentId       = p.StudentId,
        StudentFullName = p.Student?.User?.FullName ?? string.Empty,
