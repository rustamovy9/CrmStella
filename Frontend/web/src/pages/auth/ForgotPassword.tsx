...
const ForgotPassword: React.FC = () => {
    ...
    const handleSendEmail = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Пользователь не найден или ошибка почтового сервера');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleVerifyCode = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Неверный код или срок его действия истек');
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div style={styles.leftSide}>
                ...
                <div style={styles.leftContent}>
                    ...
                    <p style={styles.leftSubtitle}>
                        Наша система безопасности поможет вам быстро и безопасно восстановить доступ к учетной записи в несколько простых шагов.
                    </p>
                </div>
                ...
                <div style={styles.formWrapper}>
                    ...
                    {step < 4 && (
                        <div style={styles.stepsContainer}>
                            <div style={{...styles.stepDot, backgroundColor: step >= 1 ? '#1e40af' : '#e2e8f0'}}></div>
                            <div style={{...styles.stepLine, backgroundColor: step >= 2 ? '#1e40af' : '#e2e8f0'}}></div>
                            <div style={{...styles.stepDot, backgroundColor: step >= 2 ? '#1e40af' : '#e2e8f0'}}></div>
                            <div style={{...styles.stepLine, backgroundColor: step >= 3 ? '#1e40af' : '#e2e8f0'}}></div>
                            <div style={{...styles.stepDot, backgroundColor: step >= 3 ? '#1e40af' : '#e2e8f0'}}></div>
                        </div>
                    )}
                    ...
                    {step === 1 && (
                        <div style={styles.fadeEnter}>
                            <div style={styles.textHeader}>
                                ...
                                <p style={styles.subTitle}>Введите email, привязанный к вашему аккаунту, для получения кода подтверждения.</p>
                            </div>
                            ...
                            <form onSubmit={handleSendEmail} style={styles.form}>
                                <div style={styles.fieldContainer}>
                                    <label style={getLabelStyle(focusedField === 'email' || !!email)}>Email адрес</label>
                                    <input
                                        type="email"
                                        required
                                        value={email}
                                ...
                                <button type="submit" disabled={isLoading} style={getButtonStyle(isLoading)}>
                    ...
                    {step === 2 && (
                        <div style={styles.fadeEnter}>
                            <div style={styles.textHeader}>
                                ...
                                <p style={styles.subTitle}>Мы отправили 6-значный код на почту <b>{email}</b>.</p>
                            </div>
                            ...
                            <form onSubmit={handleVerifyCode} style={styles.form}>
                                <div style={styles.fieldContainer}>
                                    <label style={getLabelStyle(focusedField === 'code' || !!code)}>Код подтверждения</label>
                                    <input
                                        type="text"
                                        required
                                        value={code}
                                        ...
                                        style={{...getInputStyle(focusedField === 'code'), letterSpacing: '6px', fontSize: '1.3rem', textAlign: 'center'}}
                                    />
                                </div>
                                <button type="submit" disabled={isLoading} style={getButtonStyle(isLoading)}>
                    ...
                    {step === 3 && (
                        <div style={styles.fadeEnter}>
                            <div style={styles.textHeader}>
                                ...
                                <p style={styles.subTitle}>Придумайте сложный пароль, содержащий буквы и цифры.</p>
                            </div>
                            ...
                            <form onSubmit={handleResetPassword} style={styles.form}>
                                <div style={styles.fieldContainer}>
                                    <label style={getLabelStyle(focusedField === 'newPass' || !!newPassword)}>Новый пароль</label>
                                    <input
                                        type="password"
                                        required
                                        value={newPassword}
                                ...
                                <div style={styles.fieldContainer}>
                                    <label style={getLabelStyle(focusedField === 'confPass' || !!confirmPassword)}>Повторите пароль</label>
                                    <input
                                        type="password"
                                        required
                    ...
                    {step === 4 && (
                        <div style={{...styles.fadeEnter, textAlign: 'center'}}>
                            ...
                            <p style={styles.subTitle}>Ваш пароль успешно изменен. Теперь вы можете войти в личный кабинет.</p>
                            <Link to="/login" style={{textDecoration: 'none'}}>
                                <button style={{...getButtonStyle(false), marginTop: '1.5rem'}}>
...
const getLabelStyle = (isActive: boolean) => ({
    ...
    transform: isActive ? 'translateY(-22px) scale(0.85)' : 'translateY(0) scale(1)'
});
...
const styles = {
    ...
    overlayGlow: {
        ...
        top: 0,
        left: 0,
        background: 'radial-gradient(circle at 80% 20%, rgba(37,99,235,0.2), transparent 50%)',
    },
    leftHeader: {
        position: 'relative' as const,
        zIndex: 2,
